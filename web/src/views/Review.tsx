...
import { ReadingDiff, TypedAnswer } from "./review/TypedAnswer";
import { REASONS, sessionLabel, ordinal, kindLabel } from "./review/labels";
import { checkReading, type ReadingCheck } from "../../../shared/kana";
...
export function Review({
  ...
  /** The reading typed on this card, and its comparison against the truth. */
  const [typedRaw, setTypedRaw] = useState<string | null>(null);
  const [typedCheck, setTypedCheck] = useState<ReadingCheck | null>(null);
    ...
    try {
      ...
      setMnemonic(
        err instanceof Error
          ? `Could not generate one: ${err.message}`
  ...
  const heardChoices = useMemo(
    () =>
      card?.kind === "listening" && session
        ? (session.listening[card.id] ?? [])
        : [],
    [card, session],
  );
  ...
  /**
   * Cards whose answer is a reading, typed rather than picked.
   *
   * Vocab and cloze both ask for the reading of a word — typing it is active
   * recall where multiple choice was recognition. A card without a reading on
   * its back (an older cloze, say) falls back to the old behaviour untouched.
   */
  const typedReading = card?.back.reading ?? null;
  const isTypedCard =
    (card?.kind === "vocab" || card?.kind === "cloze") && typedReading !== null;
  // A typed card must not show the answer on its front: furigana and romaji
  // above the headword *are* the answer.
  const hidesReading = isTypedCard && card?.kind === "vocab";
  ...
  const isMultipleChoice =
    (card?.kind === "cloze" && choices.length > 0 && !isTypedCard) ||
    (card?.kind === "listening" && heardChoices.length > 0);
  const correctChoice =
    card?.kind === "listening" ? heardAnswer : (card?.back.answer ?? "");
  const missed =
    isMultipleChoice && chosen !== null && chosen !== correctChoice;
  ...
  /** Checks a typed reading, which also reveals the card. */
  const submitTyped = useCallback(
    (raw: string) => {
      if (!typedReading) return;
      setTypedRaw(raw);
      setTypedCheck(checkReading(typedReading, raw));
      setRevealed(true);
    },
    [typedReading],
  );
  ...
  /**
   * What this answer records in the review history: the typed reading on a
   * typed card, the picked option on a multiple-choice card.
   */
  const given = isTypedCard ? (typedRaw ?? undefined) : (chosen ?? undefined);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (!revealed && isMultipleChoice) {
        const options =
          card.kind === "listening" ? heardChoices : choices.map((c) => c.text);
      ...
      // A typed card is answered by typing: space and enter put the cursor in
      // the field rather than revealing the answer unchecked.
      if (!revealed && isTypedCard && (isSpace || e.key === "Enter")) {
        e.preventDefault();
        document.querySelector<HTMLInputElement>(".typed-row input")?.focus();
        return;
      }
    ...
    <div className="review-page">
      <div className="review-head">
        ...
        <div className="body">
          <div className="row" style={{ gap: 9 }}>
            <span className="cap">
              {options.title ?? sessionLabel(options)}
            </span>
            <span className="mono">
              {pos + 1} / {session.cards.length}
            </span>
            {card.songTitle && (
              <span className="tag jps">{card.songTitle}</span>
            )}
      ...
      <div className="review-body">
        {card.kind === "listening" && card.front.audio && (
          ...
          >
            <YouTubePlayer
              videoId={card.front.audio.youtubeId}
              onReady={setPlayer}
              scrub={false}
            />
        ...
        <div className="flashcard">
          ...
          {card.front.jp && (
            <div className={`big-jp${card.front.jp.length > 8 ? " line" : ""}`}>
              {card.front.furigana && !hidesReading ? (
                <Furigana segments={card.front.furigana} />
              ) : (
                card.front.jp
              )}
            </div>
          ...
          {card.front.romaji && card.kind !== "kana" && !hidesReading && (
            ...
            <>
              ...
              <div className="answer-row">
                <div className="who">
                  ...
                  {(card.back.reading || card.back.romaji) &&
                    card.kind !== "kana" && (
        ...
        {revealed ? (
          <>
            {isTypedCard && typedCheck && (
              <ReadingDiff check={typedCheck} expectedReading={typedReading!} />
            )}
            {missed ? (
        ...
        isMultipleChoice ? (
          <p className="muted" style={{ textAlign: "center" }}>
            Pick one — <span className="kbd">1</span>-
            <span className="kbd">4</span>.
          </p>
        ) : isTypedCard ? (
          <TypedAnswer autoFocus onSubmit={submitTyped} disabled={false} />
        ) : (
          <div className="row" style={{ justifyContent: "center" }}>
            <button
              className="primary"
