...
export function SongView({
  ...
  const [editing, setEditing] = useState<
    "reading" | "context" | "video" | "offset" | null
  >(null);
  ...
  const offlineReadings =
    (settings.data?.settings.lyric_readings ?? "ai") === "dictionary";
  ...
  const attachVideo = async () => {
    ...
    try {
      ...
      setAnalyzeMsg(
        err instanceof Error ? err.message : "Could not change the video",
      );
    }
  };
  ...
  const shiftTimings = async (ms: number) => {
    try {
      ...
      setAnalyzeMsg(
        err instanceof Error ? err.message : "Could not move the timings",
      );
    }
  };

  const markVerse = async (
    verse: VerseProgress,
  ...
  const troubleVerses = useMemo(() => {
    ...
    for (const line of lines)
      if (troubleLineIds.has(line.id)) set.add(line.verseIdx);
    return set;
      ...
      <div className="spine">
        {verses.map((verse) => (
          <button
            key={verse.verseIdx}
            className={
              verse.verseIdx === (focusVerse ?? activeVerse) ? "on" : ""
            }
          ...
          >
            ...
            {verse.verseIdx !== activeVerse &&
              troubleVerses.has(verse.verseIdx) && <span className="flag" />}
          </button>
        ))}
      </div>
      <button
  ...
  useCommands([
    ...
    },
    {
      id: "song-retime",
      label: detail?.synced
        ? "Re-time the lines by tapping"
        : "Tap to time the lines",
      where: "Song",
      run: startSync,
    },
    {
  ...
  const shownLines =
    focusVerse === null
      ? lines
      : lines.filter((l) => l.verseIdx === focusVerse);
      ...
      <div className="song-page">
            ...
            <div className="scrub">
              ...
              <button
                ...
                onClick={(e) => {
                  ...
                  player.seekMs(
                    ((e.clientX - box.left) / box.width) * durationMs,
                  );
                }}
              >
                <div className="rail" />
            ...
            {syncing ? (
              <div className="sync-hint">
                ...
                <span>
                  ...
                  <b>{Math.min(syncIdx + 1, lines.length)}</b> of {lines.length}{" "}
                  starts
                </span>
                <button
                  className="forest"
                  onClick={tapSync}
                  disabled={syncIdx >= lines.length}
                >
                  Tap
                </button>
              ...
              <div className="controls">
                ...
                <button
                  ...
                  disabled={!detail.youtubeId}
                  title={
                    detail.youtubeId ? undefined : "Stage mode needs a video"
                  }
                >
        ...
        {editing && (
          <div style={{ margin: "14px 26px 0" }}>
              ...
              <div className="card stack">
                ...
                <div className="row">
                  ...
                  <button
                    ...
                    disabled={
                      !videoInput.trim() ||
                      videoInput.trim() === detail.youtubeId
                    }
                  >
                ...
                {detail.synced && (
                  <span className="faint" style={{ fontSize: 13 }}>
                    The line timings stay as they are — if the new video starts
                    at a different point, move them all with ◷ Timings.
                  </span>
                )}
              </div>
            )}
            {editing === "offset" && (
              <OffsetEditor
                firstLineMs={
                  lines.find((l) => l.timeMs !== null)?.timeMs ?? null
                }
        ...
        <div className="song-body">
          <div className="lyrics-pane">
            ...
            {verses
              .filter(
                (verse) => focusVerse === null || verse.verseIdx === focusVerse,
              )
              .map((verse) => (
                <div key={verse.verseIdx} id={`verse-${verse.verseIdx}`}>
                  <div className="verse-head">
                    ...
                    {verse.state === "done" && (
                      <span className="tag new">DONE</span>
                    )}
                    <button
                      className="ghost small"
                      onClick={() =>
                        markVerse(
                          verse,
                          verse.state === "done" ? "in_progress" : "done",
                        )
                      }
                    >
                      {verse.state === "done"
                    ...
                    .map((line) => {
                      ...
                      return (
                        <LyricLine
                          ...
                          masteryOf={masteryOf}
                          selectedChunk={
                            selectedChunk?.lineId === line.id
                              ? selectedChunk.idx
                              : null
                          }
                          onSelectChunk={(idx) =>
                            setSelectedChunk(
                              idx === null ? null : { lineId: line.id, idx },
                            )
                          }
                          selectedToken={
                            selectedToken?.lineId === line.id
                              ? selectedToken.idx
                              : null
                          }
                          onSelectToken={(idx) =>
                          ...
                          innerRef={
                            at === activeIdx ? activeLineRef : undefined
                          }
                        />
                      );
                    })}
